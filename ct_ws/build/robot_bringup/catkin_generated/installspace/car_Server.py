import rospy
from robot_bringup.srv import C_pose,C_poseResponse
from math import pi
import math
...
TARGET_YAW = math.radians(90)  # 目标yaw角度，单位：弧度
# 定义误差容忍度和控制速度
ANGLE_TOLERANCE = 0.1  # 允许的误差容忍度，单位：弧度
ANGULAR_VELOCITY = 0.2  # 最大角速度，单位：rad/s
...
def set_goal_thera(target_theta):
    ...
    while flag_theta == False:
        ...
        if abs(yaw_error) > ANGLE_TOLERANCE:
            # 控制角速度
            cmd_vel = Twist()
            cmd_vel.angular.z = ANGULAR_VELOCITY if yaw_error > 0 else -ANGULAR_VELOCITY
            cmd_vel_pub.publish(cmd_vel)
            rospy.loginfo(f"Current yaw: {current_yaw}, Target yaw: {TARGET_YAW}, Error: {yaw_error}")
        else:
            # 目标已达到，停止旋转
            flag_theta = True
            cmd_vel = Twist()
...
if __name__ == "__main__":
    rospy.init_node('Ct_server')
    server = rospy.Service("control_Ct",C_pose,C_pose_callback )
